...
interface UWS {
    ...
    /** Sliding dedicated compress window, requires 2KB of memory per socket */
...
const uws = (() => {
    try {
        ...
        return requireModule(p) as UWS
    } catch (e) {
        throw new Error(
            `This version of uWS.js supports only Node.js 14, 16 and 18 on (glibc) Linux, macOS and Windows, on Tier 1 platforms (https://github.com/nodejs/node/blob/master/BUILDING.md#platform-list).\n\n${(
                e as object
            ).toString()}`
        )
    }
