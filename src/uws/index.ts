/* eslint-disable @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-empty-interface,@typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access */
/*
...
export interface HttpRequest {
    ...
    setYield(value: boolean): HttpRequest
}
 ...
 */
export function App(options?: AppOptions): Promise<TemplatedApp> {
    return getModule().then((m) => m.App(options) as TemplatedApp)
}

/** Constructs an SSL app. See App. */
export function SSLApp(options: AppOptions): Promise<TemplatedApp> {
    return getModule().then((m) => m.SSLApp(options) as TemplatedApp)
}

/** Closes a uSockets listen socket. */
export function us_listen_socket_close(listenSocket: us_listen_socket): Promise<void> {
    return getModule().then((m) => {
        m.us_listen_socket_close(listenSocket)
        return
    })
}

/** Gets local port of socket (or listenSocket) or -1. */
export function us_socket_local_port(socket: us_socket): Promise<number> {
    return getModule().then((m) => m.us_socket_local_port(socket) as number)
}
...
export function getParts(
    ...
    return getModule().then((m) => m.getParts(body, contentType) as MultipartField[] | undefined)
}
